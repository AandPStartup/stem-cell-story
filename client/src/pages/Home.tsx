...
const fateDetails = {
  fa: [
    { summary: "مسیر عصبی معمولاً از یک مرحله‌ی پیش‌ساز عصبی می‌گذرد. در این مسیر، ترکیب پیام‌های محیطی و فعال‌شدن ژن‌های مرتبط با شبکه‌ی عصبی، شکل سلول، اتصال‌ها و توانایی انتقال پیام را تغییر می‌دهد.", signal: "پیام‌های عصبی و تماس با بافت", role: "پردازش و انتقال پیام", note: "ساخت یک نورون کافی نیست؛ اتصال درست آن به شبکه نیز اهمیت دارد." },
    { summary: "برای رسیدن به سلول عضلانی، سلول ابتدا برنامه‌های ژنی مربوط به انقباض و سازمان‌دهی رشته‌های پروتئینی را فعال می‌کند. سپس به‌تدریج ساختاری مناسب برای تولید نیرو می‌گیرد.", signal: "برنامه‌های رشد و حرکت", role: "انقباض و تولید نیرو", note: "در بافت زنده، نظم رشته‌ها و ارتباط با عصب و خون‌رسانی نیز مهم است." },
  ...
  return <div id="top" className="site-shell" dir={direction}>
    ...
    <main>
      ...
      <section id="one" className="story-section section-grid">
        ...
        <motion.div className="fate-panel" {...motionProps}><div className="panel-heading"><span>{t.choose}</span><small dir={lang === "fa" ? "rtl" : "ltr"}>{lang === "fa" ? "از خودنوزایی تا سرنوشت سلولی" : "SELF-RENEWAL → FATE"}</small></div><div className="fate-visual"><img src={ASSETS.differentiation} alt="" /><div className="fate-core">{lang === "fa" ? "بنیادی" : "stem"}</div>{fates[lang].map((f, i) => <button key={f} className={`fate-node fate-${i + 1} ${selectedFate === i ? "selected" : ""}`} onClick={() => setSelectedFate(i)}><span className="node-dot" /><b>{f}</b></button>)}</div><AnimatePresence mode="wait"><motion.div key={selectedFate + lang} className="fate-caption fate-detail" initial={reduced ? {} : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}><div className="fate-detail-head"><span>↳</span><b>{fates[lang][selectedFate]}</b></div><p>{fateDetails[lang][selectedFate].summary}</p><div className="fate-detail-grid"><div><span>{lang === "fa" ? "راهنما" : "Guidance"}</span><b>{fateDetails[lang][selectedFate].signal}</b></div><div><span>{lang === "fa" ? "کارکرد" : "Function"}</span><b>{fateDetails[lang][selectedFate].role}</b></div></div><div className="fate-detail-note"><i />{fateDetails[lang][selectedFate].note}</div></motion.div></AnimatePresence></motion.div>
      </section>
