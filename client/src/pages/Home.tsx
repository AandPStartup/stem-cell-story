  ...
  return <div id="top" className="site-shell" dir={direction}>
    ...
    <main>
      ...
      <section id="one" className="story-section section-grid">
        <motion.div className="section-intro" {...motionProps}><SectionTag n="01" label={t.question} /><h2>{t.stemTitle}</h2><p className="lead">{t.stemIntro}</p><p>{t.stemBody}</p><div className="stem-snapshot"><div className="snapshot-head"><span>{lang === "fa" ? "داده‌ی کلیدی" : "KEY DATA"}</span><b>{lang === "fa" ? "خوانش سریع" : "QUICK READ"}</b></div><div className="snapshot-grid"><div><span>01</span><div><b>{lang === "fa" ? "منبعِ قابل نوسازی" : "Renewable source"}</b><p>{lang === "fa" ? "خودنوزایی، ذخیره‌ی سلولی را حفظ می‌کند." : "Self-renewal maintains a renewable cell source."}</p></div></div><div><span>02</span><div><b>{lang === "fa" ? "هویتِ قابل تغییر" : "Changeable identity"}</b><p>{lang === "fa" ? "پیام‌ها و ژن‌ها مسیر تخصصی‌شدن را هدایت می‌کنند." : "Signals and genes guide the route toward specialization."}</p></div></div><div><span>03</span><div><b>{lang === "fa" ? "عملکرد، معیار نهایی" : "Function is the test"}</b><p>{lang === "fa" ? "شباهت ظاهری کافی نیست؛ سلول باید درست کار کند." : "Looks are not enough; the cell must perform its role."}</p></div></div></div><div className="snapshot-foot"><i />{lang === "fa" ? "از قابلیت تا عملکرد؛ هر مرحله باید سنجیده شود." : "From potential to function; every step needs evidence."}</div></div><div className="stem-proof"><div className="proof-head"><span>{lang === "fa" ? "از هویت تا عملکرد" : "IDENTITY → FUNCTION"}</span><b>{lang === "fa" ? "سه آزمون پژوهشی" : "THREE RESEARCH CHECKS"}</b></div><div className="proof-track"><div><i>01</i><b>{lang === "fa" ? "هویت" : "Identity"}</b><p>{lang === "fa" ? "آیا نشانگرهای سلولی درست‌اند؟" : "Are the right cell markers present?"}</p></div><div><i>02</i><b>{lang === "fa" ? "خلوص" : "Purity"}</b><p>{lang === "fa" ? "آیا سلول ناخواسته باقی نمانده؟" : "Are unwanted cells minimized?"}</p></div><div><i>03</i><b>{lang === "fa" ? "عملکرد" : "Function"}</b><p>{lang === "fa" ? "آیا سلول کار واقعی خود را انجام می‌دهد؟" : "Does the cell perform its real role?"}</p></div></div><div className="proof-foot"><span />{lang === "fa" ? "ظاهر شبیه، به‌تنهایی مدرک کافی نیست." : "Looking similar is not evidence by itself."}</div></div><div className="ability-list"><div><strong>01</strong><div><h3>{t.selfRenew}</h3><p>{t.selfRenewBody}</p></div></div><div><strong>02</strong><div><h3>{t.differentiate}</h3><p>{t.differentiateBody}</p></div></div></div><button className="text-button" onClick={() => setDeep(!deep)}>{deep ? t.simpleMode : t.deepMode}<ChevronDown size={15} className={deep ? "rotate-180" : ""} /></button>{deep && <div className="deep-note">{lang === "fa" ? "تمایز به زمینه وابسته است: پیام‌های مولکولی، تماس با سلول‌های همسایه، ماتریکس بافت و تنظیم بیان ژن‌ها همگی در تعیین سرنوشت سلول نقش دارند." : "Differentiation is context-dependent: molecular signals, neighboring cells, the tissue matrix, and gene regulation all influence cell fate."}</div>}</motion.div>
